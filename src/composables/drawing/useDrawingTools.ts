...
export function useDrawingTools() {
  ...
  // Current stroke data for pencil/eraser operations
  const currentStrokeData = new Map<string, string>()

  // Track last pencil position for interpolation
  let lastPencilPosition: { x: number; y: number } | null = null
  ...
  const placeCharacter = (worldX: number, worldY: number, character: string) => {
    ...
    if (lastPencilPosition) {
      const dx = grid.x - lastPencilPosition.x
      const dy = grid.y - lastPencilPosition.y
      ...
      if (distance > 0) {
        for (let i = 0; i <= distance; i++) {
          ...
          const x = Math.round(lastPencilPosition.x + dx * progress)
          const y = Math.round(lastPencilPosition.y + dy * progress)
    ...
    lastPencilPosition = { x: grid.x, y: grid.y }
  }

  // Reset pencil tracking when stroke ends
  const resetPencilTracking = () => {
  ...
  const drawRectangle = (
    ...
    const borderStyle =
      RECTANGLE_BORDER_STYLES[borderStyleKey as keyof typeof RECTANGLE_BORDER_STYLES]
    const fillChar =
      customSettings?.fillChar !== undefined
        ? customSettings.fillChar
        : toolStore.rectangleFillChar || ''
    const shadow =
      customSettings?.shadow !== undefined
        ? customSettings.shadow
        : toolStore.rectangleShadow || false
    const text =
      customSettings?.text !== undefined ? customSettings.text : toolStore.rectangleText || ''

    // Check if features are enabled (use custom settings if provided, otherwise fall back to tool store)
    const showText =
      customSettings?.showText !== undefined ? customSettings.showText : toolStore.rectangleShowText
    const showFill =
      customSettings?.showFill !== undefined ? customSettings.showFill : toolStore.rectangleShowFill
    const showBorder =
      customSettings?.showBorder !== undefined
    ...
    const showShadow =
      (customSettings?.showShadow !== undefined
  ...
  const drawDiamond = (
    ...
    const fillChar =
      customSettings?.fillChar !== undefined
        ? customSettings.fillChar
        : toolStore.diamondFillChar || ''
    const shadow =
      customSettings?.shadow !== undefined
        ? customSettings.shadow
        : toolStore.diamondShadow || false
    const text =
      customSettings?.text !== undefined ? customSettings.text : toolStore.diamondText || ''

    // Check if features are enabled (use custom settings if provided, otherwise fall back to tool store)
    const showText =
      customSettings?.showText !== undefined ? customSettings.showText : toolStore.diamondShowText
    const showFill =
      customSettings?.showFill !== undefined ? customSettings.showFill : toolStore.diamondShowFill
    const showBorder =
      customSettings?.showBorder !== undefined
        ? customSettings.showBorder
        : toolStore.diamondShowBorder
    // Shadow requires BOTH the section checkbox (showShadow) AND the shadow toggle to be true
    const showShadow =
      (customSettings?.showShadow !== undefined
        ? customSettings.showShadow
        : toolStore.diamondShowShadow) &&
    ...
    if (showText && text) {
      ...
      const startY =
        textPosition === 'middle'
          ? centerY - Math.floor(textLines.length / 2)
          : textPosition === 'bottom'
            ? textMaxY - textLines.length + 1
            : textMinY
      ...
      textLines.forEach((line: string, i: number) => {
        ...
        if (y >= textMinY && y <= textMaxY) {
          const startX =
            textAlign === 'center'
              ? centerX - Math.floor(line.length / 2)
              : textAlign === 'right'
                ? textMaxX - line.length + 1
                : textMinX
  ...
  const drawText = (
    ...
    visibleLines.forEach((line, lineIndex) => {
      ...
      for (let i = 0; i < line.length && startX + i <= textMaxX; i++) {
        const char = line[i]
        if (char !== ' ') {
          // Don't draw spaces as actual characters
